...
#define SWITCH_DELAY 1e3  /* 1s in ms */
...
static void tup_msg_handler(TupContext *tup_ctx, TupMessage *msg,
        void *userdata)
{
...
void setup()
{
    TupMessage *tup_msg;
    uint8_t effect_id = 0;
    uint16_t effect_database_id = 0;  /* click effect */
    int ret;
    ...
    if (ret != 0) {
        Serial.print("Send msg load failed: ");
        Serial.println(ret);
        return;
    }
    ...
    ret = tup_context_send(tup_ctx, tup_msg);
    if (ret != 0) {
        Serial.print("Send msg bind failed: ");
        Serial.println(ret);
        return;
    }
    tup_message_clear(tup_msg);
    ...
    ret = tup_context_send(tup_ctx, tup_msg);
    if (ret != 0) {
        Serial.print("Send msg play failed: ");
        Serial.println(ret);
        return;
    }
    tup_message_clear(tup_msg);

    /* set threshold for release */
    tup_message_init_set_parameter_simple(tup_msg, effect_id, 0, 100);
    ret = tup_context_send(tup_ctx, tup_msg);
    if (ret != 0) {
        Serial.print("failed to set release theshold parameter ");
        Serial.println(ret);
        return;
    }
    tup_message_clear(tup_msg);

    /* set threshold for press */
    tup_message_init_set_parameter_simple(tup_msg, effect_id, 1, 1000);
    ret = tup_context_send(tup_ctx, tup_msg);
    if (ret != 0) {
        Serial.print("failed to set press theshold parameter ");
        Serial.println(ret);
        return;
    }
    tup_message_clear(tup_msg);
...
void loop()
{
    ...
    if (millis() - last_time > SWITCH_DELAY) {
        TupMessage *tup_msg;
        int ret;
        ...
        if (i % 2 == 0)
            tup_message_init_set_input_value(tup_msg, 0, 0, 1010, -1);
        else
            tup_message_init_set_input_value(tup_msg, 0, 0, 0, -1);

        Serial.println("switching button");
        ret = tup_context_send(tup_ctx, tup_msg);
        if (ret < 0)
            Serial.println("failed to send message to Tactronik");
